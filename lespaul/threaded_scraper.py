...
outfile = open('gibson-lespaul_index.csv', 'a')
...
def download_image(row, new_url, img_url):
    ...
    if r.status_code == 200:
        try:     
            client.get_object(Bucket='egil-ic', Key='gibson-lespaul/' + img_url.split('/')[-1])
            print('file exists') 
            indexwriter.writerow(['gibson-lespaul', row[2] + '_' + row[1] +'_'+ img_url.split('/')[-1]])
        except:
            client.put_object(Body=r.content, Bucket='egil-ic', Key='gibson-lespaul/' + img_url.split('/')[-1])
            indexwriter.writerow(['gibson-lespaul', row[2] + '_' + row[1] +'_'+ img_url.split('/')[-1]])
    outfile.flush()
...
# Open file
i=0
with open('lespaul.csv') as file_obj:
