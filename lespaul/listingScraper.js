import fs from 'fs'
import { lespaulQuery } from './lespaulQueries.js';
...
function processData(data) {
    ...
    for (const listing of myData.data.listingsSearch.listings) {
        ...
        // write to csv
        if (id) {
            fs.appendFile('lespaul_listings.csv', `${id},${listing.slug},${make},${model},${link + id + '-' + listing.slug}\n`, function (err) {
                if (err) throw err;
                // console.log('Saved!');
            });
        }
...
function fetchData() {
    ...
    fetch(endpoint, {
        method: 'POST',
        body: JSON.stringify(lespaulQuery(offset)),
        headers: {
